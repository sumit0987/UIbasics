loadPatientAppointmentInfo();

//Display the list of upcoming appointments for the patients
function loadPatientAppointmentInfo(){
...
function generateAppointmentSection(loggedInUserId){
    ...
    httpReq.onreadystatechange=function(){
        ...
        if(this.readyState===4 && this.status==200){
            ...
            var h2elementText = document.createTextNode('Your upcoming appointment');
            ...
            var headTd6Text = document.createTextNode('Doctor Name');
            ...
            //for(var i=0; i<datalen; i++){
                console.log(data[datalen-1].id,data[datalen-1].patientName,data[datalen-1].problem,data[datalen-1].timing,data[datalen-1].patientPhone);
                var tbodytr = document.createElement('tr');

                var tbodytrtd1 = document.createElement('td');
                var td1Text = document.createTextNode(data[datalen-1].id);
                tbodytrtd1.appendChild(td1Text);

                var tbodytrtd2 = document.createElement('td');
                var td2Text = document.createTextNode(data[datalen-1].patientName);
                tbodytrtd2.appendChild(td2Text);

                var tbodytrtd3 = document.createElement('td');
                var td3Text = document.createTextNode(data[datalen-1].problem);
               ...
               // totalOrderCost = totalOrderCost+parseInt(data[i].price);
                ...
                var td4Text = document.createTextNode(data[datalen-1].timing);
                tbodytrtd4.appendChild(td4Text);

                var tbodytrtd5 = document.createElement('td');
                var td5Text = document.createTextNode(data[datalen-1].patientPhone);
                tbodytrtd5.appendChild(td5Text);

                var tbodytrtd6 = document.createElement('td');
                var td6Text = document.createTextNode(data[datalen-1].doctorName);
                tbodytrtd6.appendChild(td6Text);
    ...
    httpReq.open('get','http://localhost:3001/appointments?patientId='+loggedInUser,true);
    httpReq.send();
}
